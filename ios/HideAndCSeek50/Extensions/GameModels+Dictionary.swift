...
// MARK: - QuestionSet Dictionary Extensions
// Explicit implementations store categories and questions as keyed dictionaries
// (using their id as the key) rather than JSON arrays. Firebase RTDB converts
// JSON arrays to {"0":…,"1":…} dictionaries on round-trip, which breaks generic
// Codable decoding. The CardDeck module uses the same keyed approach successfully.
...
extension QuestionSet {
    ...
    static func fromDictionary(_ dict: [String: Any]) throws -> QuestionSet {
        guard let id = dict["id"] as? String,
            ...
            throw DatabaseError.invalidData("QuestionSet.fromDictionary: missing required fields")
        }
...
extension QuestionCategoryDef {
    ...
    static func fromDictionaryWithOrder(_ dict: [String: Any]) throws -> (Int, QuestionCategoryDef) {
        guard let id = dict["id"] as? String,
              ...
              let keepCount = dict["keepCount"] as? Int,
              let timeLimitSeconds = dict["timeLimitSeconds"] as? Int else {
            throw DatabaseError.invalidData("QuestionCategoryDef.fromDictionary: missing required fields")
        }
        ...
        return (orderIndex, QuestionCategoryDef(
            id: id,
            name: name,
            iconName: iconName,
            drawCount: drawCount,
...
extension CustomQuestion {
    ...
    static func fromDictionaryWithOrder(_ dict: [String: Any]) throws -> (Int, CustomQuestion) {
        guard let id = dict["id"] as? String,
            ...
            throw DatabaseError.invalidData("CustomQuestion.fromDictionary: missing required fields")
        }

        let orderIndex = dict["orderIndex"] as? Int ?? 0

        // choices may be stored as an array or as a Firebase numeric-keyed dict
        var choices: [String] = []
        if let choicesArray = dict["choices"] as? [String] {
            choices = choicesArray
        } else if let choicesDict = dict["choices"] as? [String: String] {
            // Restore ordering from numeric string keys
            choices = choicesDict.keys
                .compactMap { Int($0) }
                .sorted()
                .compactMap { choicesDict[String($0)] }
        }

        return (orderIndex, CustomQuestion(
            id: id,
            text: text,
            questionType: questionType,
            choices: choices
